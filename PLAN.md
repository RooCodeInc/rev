...
| 4.6 | MLX (bf16) vs fp32 torch on **all 1,024 clean development records of decision-v7 (1,264 questions)**, Kev-0.8B: **4 argmax flips (0.32 %)**, max \|Δp\| 0.054, mean 0.0023, 11–13 questions above 0.02 (full / prefix / alone paths agree). The published 40–60-record figure (max 0.011–0.024, 0–1 flips) understated the tail. Kev-4B running. [`runs/r4-mlx-parity-0.8b`](runs/r4-mlx-parity-0.8b/report.json) | Reporting; the cards and README get the full-partition numbers when the 4B read lands. |
...
Pending: 4.6 (4B), 4.8(b) (running, `r4-q35-9b-inst2`), 4.9 and 4.11 (`r4-deltas` hard controls, `r4-soft` soft arms), 4.12 delta (`r4-longstate`), all on the isolated app `kev-round4`.
