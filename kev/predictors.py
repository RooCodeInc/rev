...
class RotationAveraged:
    ...
    def __call__(self, record):
        ...
        probabilities: dict = {}; logits: dict = {}
        out = {"probabilities": probabilities, "latency_ms": sum(p["latency_ms"] for p in preds), "rotations": len(preds)}
        if field == "logits":
            out["logits"] = logits; out["inference_temperature"] = preds[0].get("inference_temperature", 1.0)
        for qid in record["questions"]:
            ...
            probabilities[qid] = {k: v / s for k, v in e.items()}
            if field == "logits": logits[qid] = z
        return out
