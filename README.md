...
On Apple Silicon there are no PyTorch kernels for the DeltaNet layers, so the server runs the Qwen3.5 models through [MLX](https://github.com/ml-explore/mlx-lm) instead (`uv sync --extra serve` installs it on Macs). Only the backbone changes. Kev's encoder, the pointer head and the calibration are the same code, and the probabilities match the fp32 PyTorch path to bf16 rounding. On 40 Kev-4B development records the largest difference is 0.024 and the mean 0.004, with no change in the highest-probability answer; on 60 Kev-0.8B records the largest is 0.017, with one flip on a question the fp32 path had at 0.299 against 0.297. Median time on an M5 (32 GB) for five questions with three options each on a ~270-token state, through the model directly:
